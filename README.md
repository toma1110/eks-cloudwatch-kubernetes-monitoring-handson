...
OSやPCの管理権限によって手順が変わるため、迷った場合は各ツールの公式手順を優先してください。
...
1. Bashスクリプトを実行できるように、Git for WindowsまたはWSLを用意します。
2. AWS CLI v2はWindows用MSIインストーラーでインストールします。
3. kubectl、eksctl、Helm、Terraform、jqをそれぞれ公式手順に沿ってインストールし、実行ファイルを `PATH` に追加します。
4. 新しいターミナルを開き直して、後述のバージョン確認を実行します。
